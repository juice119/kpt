...
function runGh(args) {
  ...
  if (res.status !== 0) {
    console.warn(`[gh 경고] ${args.join(' ')} 실패: ${res.stderr?.trim() || res.error?.message}`);
    return [];
  }
  try {
...
function fetchCommits(date) {
  ...
  return items.map((c) => `- [${c.repository?.fullName ?? '?'}] ${c.commit?.message?.split('\n')[0] ?? ''}`);
}
...
function fetchPrs(date) {
  ...
  return items.map((p) => `- [${p.repository?.fullName ?? '?'}] (${p.state}) ${p.title} ${p.url}`);
}

async function askEnvOrPrompt(envName, label) {
  let value = process.env[envName];
  if (!value) {
    console.log(`${envName} 환경변수 없음. (다음부터는 export ${envName}=... 해두면 이 프롬프트 건너뜀)`);
...
async function fetchToggl(date) {
  const token = await askEnvOrPrompt('TOGGL_API_TOKEN', 'Toggl API 키 입력 (focus.toggl.com/settings 에서 발급, toggl_sk_...)');
  if (!token) {
    console.warn('[toggl 경고] 토큰 미입력, Toggl 데이터 건너뜀');
    return [];
  }
  const orgId = await askEnvOrPrompt('TOGGL_ORG_ID', 'Toggl organization_id 입력 (focus.toggl.com 접속 시 URL에서 확인)');
  const workspaceId = await askEnvOrPrompt('TOGGL_WORKSPACE_ID', 'Toggl workspace_id 입력 (focus.toggl.com 접속 시 URL에서 확인)');
  if (!orgId || !workspaceId) {
    console.warn('[toggl 경고] organization_id/workspace_id 미입력, Toggl 데이터 건너뜀');
    return [];
  }
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[toggl 경고] API 요청 실패: ${res.status} ${res.statusText}`);
      return [];
    }
    ...
    return entries.map((e) => {
      ...
      const title = e.description || e.task?.name || e.project?.name || '(제목 없음)';
...
function callClaude(prompt) {
  ...
  if (res.status !== 0) {
    throw new Error(`claude 호출 실패: ${res.stderr?.trim() || res.error?.message}`);
  }
